...
template<typename T>
std::vector<int>* SparseMatrix<T>::getColum(void)  const
{
    return this->cols;
}

template<typename T>
std::vector<int>* SparseMatrix<T>::getRow(void) const
{
	return this->rows;
}
...
std::vector<T> SparseMatrix<T>::multiply(const std::vector<T> & x) const
{
    if (this->n != (int) x.size()){
        std::cerr<< "Cannot multiply: Matrix column count and vector size don't match." << std::endl;
    }
...
{
    if (this->n != m.m){
        std::cerr << "Cannot multiply: Left matrix column count and right matrix row count don't match." << std::endl;
    }
...
SparseMatrix<T> SparseMatrix<T>::add (const SparseMatrix<T> & m) const
{
    if (this->m != m.m || this->n != m.n){
        std::cerr<< "Cannot add: matrices dimensions don't match."<<std::endl;
    }
...
{
    if (rows < 1 || columns < 1){
        std::cerr << "Matrix dimensions cannot be zero or negative." << std::endl;
    }
